...
- отправка файла с именем `cookie.txt` — обновляет cookies в контейнере без рестарта. Для этого `YTDLP_COOKIES_FILE` должен указывать на путь, доступный на запись пользователю `app`, например `/data/cache/cookies.txt` (файл кладётся в volume `ttblow-cache`).
